...
%install
	...
	for d in interfaces services zones rules
	do
	...
	for f in audit debug error traffic vuurmuur
	do
...
%post
	for service in '%{_datadir}/vuurmuur/services'/*
	do
		basename=$(basename "${service}")
		if [[ ! -e "%{_sysconfdir}/vuurmuur/textdir/services/${basename}" ]]
		then
